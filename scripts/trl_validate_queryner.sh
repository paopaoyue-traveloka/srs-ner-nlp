#!/usr/bin/env bash
# TRL + MiniCPM5 评估脚本（在 test 集上评估已训练的 LoRA adapter）
#
...
#   bash scripts/trl_validate_queryner.sh --model_dir /path/to/adapter
#
# 环境变量（可选）：
#   CUDA_VISIBLE_DEVICES=0     指定 GPU
#   BASE_MODEL=openbmb/MiniCPM5-1B  模型名或本地路径
#   MODEL_DIR=                 模型目录（默认 .model/queryner/trl/best）
